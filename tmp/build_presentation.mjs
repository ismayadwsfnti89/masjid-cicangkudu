...
const FINAL_PPTX = path.join(workspaceDir, 'output', 'pptx', 'Presentasi_Sistem_Masjid_Cicangkudu.pptx');
...
{ const s=p.slides.add(); header(s,4,'Alur kerja admin','Admin mengelola konten, data, pembayaran, dan verifikasi dalam dashboard.');
  ...
  card(s,'Kelola keuangan','Catat pemasukan dan pengeluaran. Donasi diterima menjadi pemasukan secara otomatis.',708,420,500,165,C.green); s.speakerNotes.textFrame.setText('Jelaskan tugas rutin admin dan tekankan bahwa hapus massal meminta konfirmasi sebelum data dihapus.'); }
{ const s=p.slides.add(); header(s,5,'Struktur sistem dan data','Aplikasi Laravel memisahkan tampilan, rute, proses bisnis, dan basis data.');
  const cols=[['Tampilan','Blade admin dan warga','Dashboard, formulir, kartu informasi, profil, notifikasi'],['Rute','routes/web.php','Menghubungkan URL, middleware admin, dan controller'],['Proses','Controller + Model','CRUD, verifikasi donasi, API jadwal, neraca kas'],['Data','MySQL + storage','Akun, konten, donasi, profil, notifikasi, dan gambar']];
  cols.forEach((c,i)=>card(s,c[0],`${c[1]}\n\n${c[2]}`,72+i*285,230,250,255,i===1?C.gold:C.green));
  text(s,'API jadwal salat memakai data Kab. Tasikmalaya, Jawa Barat (wilayah Cicangkudu).',72,540,1080,34,17,C.green,true); s.speakerNotes.textFrame.setText('Perlihatkan struktur empat lapisan. Sebutkan bahwa API jadwal disesuaikan dengan Kab. Tasikmalaya.'); }
{ const s=p.slides.add(); header(s,6,'Donasi terverifikasi dan Neraca Kas','Pencatatan dibuat berbasis kas agar mudah dipahami dan siap dikembangkan.');
  const y=235; [['Pemasukan','Donasi diterima + transaksi pemasukan',C.green],['Pengeluaran','Transaksi pengeluaran',C.gold],['Kas dan Bank','Pemasukan − Pengeluaran',C.green],['Dana Bersih','Kas dan Bank − kewajiban',C.green]].forEach((a,i)=>{ const x=72+i*285; shape(s,'roundRect',x,y,250,175,C.white,C.line); shape(s,'roundRect',x+20,y+20,210,34,a[2],a[2]); text(s,a[0],x+32,y+28,188,17,14,C.white,true,'center'); text(s,a[1],x+25,y+75,200,58,14,C.dark,true,'center'); });
  text(s,'Dalam versi ini kewajiban bernilai Rp0, sehingga Aset Kas = Dana Bersih.',72,500,1060,36,20,C.green,true); text(s,'Jika nanti ada utang atau aset tetap, modul dapat diperluas tanpa mengubah alur transaksi kas.',72,548,1040,30,15,C.muted); s.speakerNotes.textFrame.setText('Terangkan rumus neraca kas. Tekankan bahwa sistem sekarang menggunakan basis kas dan bisa dikembangkan untuk kewajiban atau aset tetap.'); }
{ const s=p.slides.add(); header(s,7,'Hasil akhir dan langkah demo','Sistem siap digunakan sebagai dasar pengelolaan masjid yang lebih tertib.');
  card(s,'1. Demo admin','Tambahkan informasi, ubah profil masjid, atur QRIS, verifikasi donasi, dan lihat neraca kas.',72,230,520,210,C.green);
