...
OUT = r"C:\masjid-cicangkudu\output\pdf\Dokumentasi_Sistem_Masjid_Cicangkudu_Revisi.pdf"
...
story += [P('Alur Sistem', 'H1Green'), make_table(['Tahap', 'Alur'], [['1. Autentikasi', 'Pengguna login. Middleware membatasi halaman admin hanya untuk role admin.'], ['2. Informasi', 'Admin membuat informasi atau kegiatan dan memilih status terbit. Konten tampil pada warga dan menghasilkan notifikasi.'], ['3. Donasi', 'Warga memilih program, mengunggah bukti transfer, lalu sistem membuat data berstatus pending.'], ['4. Verifikasi', 'Admin menerima atau menolak bukti. Donasi diterima tercatat otomatis sebagai Donasi Masuk.'], ['5. Laporan', 'Admin mencatat pengeluaran. Sistem menghitung saldo dari Donasi Masuk dikurangi pengeluaran.']], [3.2*cm, 14.2*cm])]
story += [P('Struktur Kode', 'H1Green'), make_table(['Bagian', 'File Penting', 'Tanggung Jawab'], [['Routing', 'routes/web.php', 'Mendaftarkan halaman warga, admin, CRUD konten, verifikasi donasi, dan hapus massal.'], ['Konten Admin', 'AdminContentController.php', 'CRUD informasi, kegiatan, donasi, laporan keuangan, serta hapus massal konten.'], ['Donasi', 'DonationController.php dan AdminDonationController.php', 'Unggah bukti, pemberitahuan admin, menerima atau menolak donasi.'], ['Keuangan', 'LaporanController.php', 'Menghitung donasi masuk, pengeluaran, dan saldo.'], ['Profil Masjid', 'MasjidProfileController.php', 'Menyimpan foto, biodata, visi, dan misi yang tampil pada warga.'], ['Profil Warga', 'WargaProfileController.php', 'Memperbarui nama, kata sandi, dan foto profil warga.']], [2.5*cm, 5.0*cm, 9.9*cm])]
...
story += [P('Pengelolaan Laporan Keuangan', 'H1Green'), P('Laporan hanya menampilkan Donasi Masuk, Pengeluaran, dan Saldo. Donasi Masuk berasal dari bukti donasi yang telah diverifikasi admin. Admin menambah pengeluaran secara manual. Saldo dihitung dari Donasi Masuk dikurangi Pengeluaran. Uang kotak amal dan fasilitas atau inventaris milik santri tidak dihitung sebagai kas laporan masjid.'), make_table(['Komponen', 'Sumber atau Rumus'], [['Donasi Masuk', 'Donasi warga yang sudah diverifikasi oleh admin.'], ['Pengeluaran', 'Catatan pengeluaran yang dibuat admin.'], ['Saldo', 'Donasi Masuk - Pengeluaran.']], [5.0*cm, 12.4*cm])]
story += [P('Panduan Admin', 'H1Green'), make_table(['Menu', 'Langkah'], [['Informasi dan Kegiatan', 'Tambah data, pilih jenis konten, isi judul, deskripsi, tanggal, gambar, dan status terbit. Gunakan Profil Masjid untuk mengubah kartu biodata warga.'], ['Program Donasi', 'Tambah program, atur rekening dan QRIS, lalu periksa bukti transfer yang masuk.'], ['Laporan Keuangan', 'Catat pengeluaran. Donasi yang diverifikasi masuk otomatis sebagai Donasi Masuk. Pantau Donasi Masuk, Pengeluaran, dan Saldo.'], ['Data Warga', 'Tambah manual, impor Excel, edit, hapus satu data, atau pilih beberapa warga untuk hapus massal.'], ['Hapus Massal', 'Centang data yang diperlukan, klik Hapus data terpilih, lalu konfirmasi. Tindakan tidak dapat dibatalkan.']], [4.0*cm, 13.4*cm])]
